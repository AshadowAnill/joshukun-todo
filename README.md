...
- 5個達成すると自動でスタンプ帳に切替＋今日のセルにスタンプが押される
...
```json
{
  ...
  "switched": true,
  "calendar": { "2026-05-11": true, "2026-05-12": true }
}
```

- `checked`: 今日のチェック状況。日付が変わるとリセット
- `switched`: 今日達成済みフラグ。自動切替の再発火防止用。日付変更でリセット
- `calendar`: 達成日の永続記録。日付変更でもクリアされない

旧 v1 データは初回ロード時に自動的に v2 に変換される。v1 で当日達成済みだった場合、カレンダー側に記録が引き継がれる。
